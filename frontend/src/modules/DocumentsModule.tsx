...
export default function DocumentsModule() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 overflow-hidden">
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto fluent-scrollbar p-4 space-y-3">
        <header>
        ...
        {documentInventoryError && (
          <div
            className="flex items-center gap-2 text-[12px] px-2.5 py-1.5 rounded-sm border border-solid"
        ...
        <div
          ...
          className={`rounded-sm border-2 border-dashed p-6 text-center cursor-pointer fluent-transition ${
        ...
        >
          ...
          <p className="text-[11px] font-medium text-[var(--ws-text-secondary)] mt-0.5">
        ...
        {uploadQueue.length > 0 && (
          <ul className="space-y-1">
            {uploadQueue.map((q) => (
              <li key={q.name} className="flex items-center gap-2 text-[12px] ws-card px-2.5 py-1.5">
        ...
        <input
          ...
          className="ws-input-bar w-full px-2.5 py-1.5 text-[13px] focus:outline-none"
        />

        {documentInventoryLoading && documentInventory.length === 0 ? (
          <div className="flex items-center gap-2 text-[12px] text-[var(--ws-text-secondary)] py-6 justify-center">
          ...
          <div className="ws-card overflow-x-auto">
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-2.5 py-6 text-center text-[var(--ws-text-secondary)]">
                  ...
                  filtered.map((row) => (
                    <tr
                      key={row.filename}
                      className="border-b cursor-pointer ws-row-interactive"
                      style={{ borderColor: "var(--ws-border)" }}
                      onClick={() => setSelected(row)}
                    >
                      <td className="px-2.5 py-1.5 max-w-[200px] truncate">{row.filename}</td>
                      <td className="px-2.5 py-1.5">{row.type}</td>
                      <td className="px-2.5 py-1.5">{row.chunks}</td>
                      <td className="px-2.5 py-1.5">{row.embedding_dimensions} dim</td>
                      <td className="px-2.5 py-1.5">{row.indexation_state}</td>
                      <td className="px-2.5 py-1.5 text-[var(--ws-text-secondary)]">
                      ...
                      <td className="px-2.5 py-1.5">
                        <button
                          ...
                          className="p-1 rounded-sm hover:text-[var(--ws-danger)]"
                        >
      ...
      {selected && (
        <aside className="w-72 shrink-0 border-l p-3 overflow-y-auto fluent-scrollbar" style={{ borderColor: "var(--ws-border)" }}>
          <h2 className="text-[12px] font-semibold mb-2">Document Inspector</h2>
          <dl className="text-[11px] space-y-1.5 text-[var(--ws-text-secondary)]">
